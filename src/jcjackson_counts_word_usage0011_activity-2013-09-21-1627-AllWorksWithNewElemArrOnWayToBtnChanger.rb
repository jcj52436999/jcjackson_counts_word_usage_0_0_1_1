...
last backup save was jcjackson_counts_word_usage0011_activity-2013-09-21-1627-AllWorksWithNewElemArrOnWayToBtnChanger.rb
...
class JcjacksonCountsWordUsage0011Activity
  def onCreate(bundle)
    super
    set_title 'JC Jackson Counts Word Usage - 0.0.1.1h'
  ...
  def onCreateContextMenu(menu, v, menuInfo)
    ...
    menu.add(0, v.id, 0, "ContextAction-1, id=" + v.id.to_s).setOnMenuItemClickListener do |menu_item|
    ...
    menu.add(0, v.id, 0, 'ContextAction-2, id=' + v.id.to_s).setOnMenuItemClickListener do |menu_item|
      # @text_view.text = menu_item.title
      # toast menu_item.title
    ...
    menu.add(0, v.id, 0, 'ContextAction-3, id=' + v.id.to_s).setOnMenuItemClickListener do |menu_item|
      # @text_view.text = menu_item.title
      # toast menu_item.title
      @textViewTail = menu_item.title.to_s
      aString = makeDisplayString(@btnLn, @btnStr, @btnCnt, @textViewTail)
