...
class JcjacksonCountsWordUsage0011Activity
  def onCreate(bundle)
    super
    set_title 'JC Jackson Counts Word Usage - 0.0.1.1j'
  ...
  def onCreateContextMenu(menu, v, menuInfo)
    ...
    menu.add(0, v.id, 0, '@menuChoiceEntry.text, id=' + v.id.to_s).setOnMenuItemClickListener do |menu_item|
      # @text_view.text = menu_item.title
      # toast menu_item.title
